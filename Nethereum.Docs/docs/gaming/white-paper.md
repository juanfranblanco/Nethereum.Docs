...
Online gaming currently relies exclusively on siloed gaming platforms serving as trusted third parties to process electronic payments and transactions between players. While the system works well enough within proprietary networks, it still suffers from the inherent limitations of the customer lock-in based model: hardware manufacturers centralize payments and gamers data, effectively building closed-gardens. These enclosed "game economies" won't allow players to accrue reputation, trade items or simply interact across gaming platforms. Meanwhile, the current siloed model hinders opportunities such as micro-payment or pay per-minute because their reliance on traditional fiat payment systems.

What is needed is a decentralized platform connected to every gaming network. If it relies on This would enable cors
